...
void sf2d_draw_quad_uv(const sf2d_texture *texture, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float u1, float v1, float u2, float v2, float u3, float v3, float u4, float v4, u32 color, unsigned int params)
{
	...
	vertices[0].position = (sf2d_vector_3f){x1, y1, SF2D_DEFAULT_DEPTH};
        vertices[1].position = (sf2d_vector_3f){x2, y2, SF2D_DEFAULT_DEPTH};
        vertices[2].position = (sf2d_vector_3f){x3, y3, SF2D_DEFAULT_DEPTH};
        vertices[3].position = (sf2d_vector_3f){x4, y4, SF2D_DEFAULT_DEPTH};

        vertices[0].texcoord = (sf2d_vector_2f){u1, v1};
        vertices[1].texcoord = (sf2d_vector_2f){u2, v2};
        vertices[2].texcoord = (sf2d_vector_2f){u3, v3};
        vertices[3].texcoord = (sf2d_vector_2f){u4, v4};
